...
我们提供了一个扩展函数 `extendMd`，以供你快速扩展页面信息。

在主题的 `valaxy.config.ts` 中，你可以通过 `extendMd` 来访问每个 Markdown 页面的路由、frontmatter 数据、摘要和文件路径，并在构建时对其进行修改。
...
export default defineTheme({
  extendMd(ctx) {
    // ctx.route - EditableTreeNode，可修改路由 meta 信息
    // ctx.data  - 只读的 frontmatter 原始数据
    // ctx.content - 原始 Markdown 内容
    // ctx.excerpt - 摘要内容（如果存在）
    // ctx.path  - Markdown 文件的绝对路径
...
> 你也可以使用你自己的全局状态管理。参见 [全局状态管理](#global-state-management)。
...
### NProgress 进度条 {#nprogress-progress-bar}
...
#### 获取用户的 Valaxy Config {#get-user-s-valaxy-config}
...
#### 提供 Typed useThemeConfig {#provide-typed-usethemeconfig}
...
#### 获取文章分类与标签 {#get-post-categories-and-tags}
...
#### 获取 Front-matter {#get-front-matter}
...
### 添加依赖预构建 `optimizeDeps` {#add-dep-pre-bundling-optimizedeps}
...
> [为什么用 dayjs 而不是 date-fns？](/dev/notes/app-bundle-size#date-fns-vs-dayjs?)
