...
The Arknights example implements this at `theme/components/ArknightsMobileNav.vue`. Its bilingual usage guide is https://valaxy.site/themes/arknights (Chinese: https://valaxy.site/zh/themes/arknights). The live example is https://arknights.valaxy.site/. The theme is published as `valaxy-theme-arknights` on npm; install it with `pnpm add valaxy-theme-arknights`, or use a packed archive when validating local changes. Do not add full-page section navigation, particle fields or loading controllers unless the blog needs them.
