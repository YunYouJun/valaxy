...
<template>
  <!-- Loading -->
  <div v-if="configLoading && !configData" class="h-full flex items-center justify-center text-sm op-50">
    {{ t('config.loading') }}
  </div>

  <!-- No config data -->
  <div v-else-if="!configData" class="h-full flex items-center justify-center text-sm op-50">
    {{ t('config.no_data') }}
  </div>
  ...
  <Splitpanes v-else class="h-full">
    ...
    <Pane min-size="30" size="45">
      <div class="h-full flex flex-col">
        ...
        <div class="flex gap-1 px-3 pt-3 pb-1 flex-wrap">
          <button
        ...
        <!-- Quick Edit content (scrollable) -->
        <div class="flex-1 overflow-auto p-3">
          <!-- Site Info Tab -->
          <div v-if="quickTab === 'site'" class="flex flex-col gap-4">
            <ConfigFieldInput
              config-type="site"
              field="title"
          ...
          <!-- Author Tab -->
          <div v-else-if="quickTab === 'author'" class="flex flex-col gap-4">
            <ConfigFieldInput
              config-type="site"
              field="author.name"
          ...
          <!-- Features Tab -->
          <div v-else-if="quickTab === 'features'" class="flex flex-col gap-4">
            ...
            <ConfigFieldGroup :title="t('config.group_general')" icon="i-ri:settings-3-line">
              <ConfigFieldCheckbox
                config-type="site"
                field="lastUpdated"
          ...
          <!-- Build Tab -->
          <div v-else-if="quickTab === 'build'" class="flex flex-col gap-4">
            <ConfigFieldInput
              config-type="valaxy"
              field="theme"
          ...
          <!-- Theme Tab -->
          <div v-else-if="quickTab === 'theme'" class="flex flex-col gap-4">
    ...
    <!-- Right Pane: All Options (reference) -->
    <Pane>
      <div class="h-full flex flex-col">
        <!-- Right header -->
        <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-100 dark:border-gray-800">
          ...
          <span class="text-xs op-40 tabular-nums">{{ filteredFields.length }}</span>
        </div>
        ...
        <div class="flex-1 overflow-auto px-2 pb-2">
          <div class="flex flex-col gap-1.5 pt-1">
            ...
            >
              ...
              >
                <div
                  class="i-ri:arrow-right-s-line text-xs transition-transform"
                ...
                >
                  <div class="flex items-start gap-1.5">
                    <div class="flex-1 min-w-0">
                      ...
                      <p v-if="getDescription(field)" class="text-xs op-60 mt-0.5 leading-relaxed">
